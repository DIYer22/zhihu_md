...
def image_ops(_lines):
    _lines = re.sub(
        r"\!\[(.*?)\]\((.*?)\)",
        functools.partial(rename_image_ref, original=True),
        _lines,
    )
    _lines = re.sub(
        r'<img src="(.*?)"', functools.partial(rename_image_ref, original=False), _lines
    )
    return _lines
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-i", "--input", type=str, help="Path to the file you want to transfer."
    )
